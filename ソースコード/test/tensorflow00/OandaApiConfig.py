...
def OpenOrder():
    open_orders = oanda.get_orders(account_id)
...
def HistricalTrade(count):
    trade_hist = oanda.get_transaction_history(account_id,count= count)
...
def LimitOrder(LimitTime,Price,Units,Side):
    limit_order = oanda.create_order(account_id,
                                     instrument = "USD_JPY",
                                     price = Price,
                                     units = Units,
                                     side = Side,
...
def ChangeOrder(Order_id,Price,Units):
    changeorder = oanda.modify_order(account_id,
                                     instrument = "USD_JPY",
                                     order_id=Order_id,
                                     price=Price,
                                     units=Units)
